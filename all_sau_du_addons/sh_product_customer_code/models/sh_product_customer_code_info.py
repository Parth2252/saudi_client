...
class ShProductCustomerCode(models.Model):
    ...
    def _check_duplicate_customer_code(self):
        for rec in self:
            if rec.product_code:
                ...
                if duplicate:
                    raise ValidationError(
                        _("The Customer Product Code '%s' already exists for this customer in another product.")
                        % rec.product_code
                    )
    ...
    #     if vals.get('name') and vals.get('product_code'):
    ...
    #         if duplicates:
    #             raise ValidationError(
    #                 _("This customer product code already exists for the selected customer.")
    #             )
    #     return super(ShProductCustomerCode, self).create(vals)
    #
    # def write(self, vals):
    #     for rec in self:
    #         customer_id = vals.get('name', rec.name.id)
    #         product_code = vals.get('product_code', rec.product_code)
    #
    #         duplicates = self.search([
    #             ('id', '!=', rec.id),
    #             ('name', '=', customer_id),
    ...
    #                 _("This customer product code already exists for the selected customer.")
    #             )
    #     return super(ShProductCustomerCode, self).write(vals)